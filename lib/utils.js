...
var extend = exports.extend = deprecate.function () {
  var o, a, b, p, i;
    ...
    return eachAsync(files.sort(), function (file, next) {
      ...
      fs.stat(fp, function (err, stats) {
        ...
        if (stats.isDirectory()) {
          if (!recursive) return next();

          requireDir(fp,cb,exts,true).then(function () {
            next();
          },next);

        } else if (stats.isFile()) {
          // if the file starts with '.' or doesn't has an extension.
...
function requireFile (file) {
  ...
  if (ext === '.json') {
    ...
    return cson.parseFileSync(file, {sandbox:{}});
